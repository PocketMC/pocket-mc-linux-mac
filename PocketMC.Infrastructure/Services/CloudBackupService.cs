            ...
            IProgress<double>? progress = null)
        {
            ...
            foreach (var provider in _providers)
            {
                ...
                if (status != CloudBackupConnectionStatus.Connected)
                {
                    ...
                    _logger.LogInformation("Skipping cloud backup upload to {Provider} because connection status is {Status}.", provider.ProviderType, status);
                    continue;
                }
                ...
                uploadTasks.Add(UploadToProviderAsync(provider, target, instanceId, instanceName, localZipPath, fileName, sha256, onProgress, progress));
            }
