...
if [ "$1" = "uninstall" ] || [ "$1" = "--uninstall" ]; then
  ...
  if [ "$OS" = "Linux" ]; then
    ...
    echo -e "${GREEN}✓ PocketMC successfully uninstalled from Linux.${NC}"
  elif [ "$OS" = "Darwin" ]; then
    rm -rf "/Applications/PocketMC.app" "$HOME/Applications/PocketMC.app"
    rm -f "$HOME/.local/bin/pocketmc"
    echo -e "${GREEN}✓ PocketMC successfully uninstalled from macOS.${NC}"
  fi
  exit 0
fi
...
echo -e "✓ OS: ${BOLD}$OS${NC} | Architecture: ${BOLD}$ARCH${NC} ($TARGET)"

# Dependency check
command -v curl >/dev/null 2>&1 || { echo -e "${RED}❌ 'curl' is required but not installed.${NC}"; exit 1; }

if [ "$EXT" = "tar.gz" ]; then
  command -v tar >/dev/null 2>&1 || { echo -e "${RED}❌ 'tar' is required but not installed.${NC}"; exit 1; }
elif [ "$EXT" = "zip" ]; then
...
if [ -z "$LATEST_TAG" ]; then
  echo -e "${BLUE}ℹ️  Could not query GitHub release API. Falling back to version lookup...${NC}"
  LATEST_TAG="v1.0.0"
fi
...
EOF
  ...
  echo -e ""
  echo -e "${GREEN}${BOLD}🎉 PocketMC successfully installed on Linux!${NC}"
  echo -e "   - Executable: ${BOLD}$BIN_DIR/pocketmc${NC}"
  echo -e "   - Desktop App: Added to system application menu (${BOLD}pocketmc.desktop${NC})"
  
  if [[ ":$PATH:" != *":$BIN_DIR:"* ]]; then
    echo -e ""
    echo -e "${BLUE}💡 Note: Make sure $BIN_DIR is in your PATH. Add this line to your shell profile (~/.bashrc or ~/.zshrc):${NC}"
  ...
  echo -e ""
  echo -e "${GREEN}${BOLD}🎉 PocketMC successfully installed on macOS!${NC}"
  echo -e "   - App Bundle: ${BOLD}$TARGET_APP_DIR/PocketMC.app${NC}"
  echo -e "   - You can launch PocketMC directly from Spotlight or Launchpad."
fi
